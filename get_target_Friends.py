...
def get_info(user_id, offset, ids):
    target_friends = requests.get("https://api.vk.com/method/friends.get?&user_id={0}&fields=first_name,last_name,city&count=5000&offset={1}&access_token={2}&v=5.131".format(user_id, offset, access_token_friends))
...
def Friends():
    ...
    offset = 0
    count = requests.get("https://api.vk.com/method/friends.get?&user_id={0}&fields=first_name,last_name,city&count=5000&offset={1}&access_token={2}&v=5.131".format(user_id, offset, access_token_friends)).json()["response"]["count"]
    ids = input("Would one like to get cities and IDs (y/n)? ").lower()
    while offset < count:
        get_info(user_id, offset, ids)
        time.sleep(0.1)
        offset += 5000
    print("Total amount of target \033[1;94mFriends \033[1;00mis\033[1;95m", count, "\033[1;00m")
    compare_results()
